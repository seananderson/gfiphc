...
check_iphc_spp_name <- function(countData = NULL, ignore_obvious = TRUE) {
  ...
  if (!is.null(countData)) {
    ...
    data_names_2023 <- unique(as.character(countData2023$spNameIPHC))
    data_names_all <- c(
      data_names_1995,
...
##' Get the data for IPHC 2013 or 2020 to 2023 surveys for a given species, or later
...
##'
##' Details
##' @param year year of interest, must be 2013, 2020, 2021, 2022, or 2023
##' @return Tibble contains year, station name, lat, lon,
##'           E_it (effective skate number for that station, based on all
##'                   hooks, so all NA),
...
get_iphc_from_gfiphc <- function(species,
                                 year){
  stopifnot(year %in% c(2013, 2020, 2021, 2022, 2023))  # update this each year
  iphc_spp_name <- get_iphc_spp_name(species)
...
##'   NA for 1997-2002, 2013, 2020, 2021, 2022, 2023),
##'  * N_it...NUM - number of 'species' caught on all hooks, so NA for
##'   1997-2002, 2013, 2020, 2021, 2022, 2023, where NUM is the column number appended by dplyr within this
##'   function,
...
##'   based on all hooks, so NA for 1997-2002, 2013, 2020, 2021, 2022, 2023
